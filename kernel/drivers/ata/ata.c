...
int ata_read(int bus, int drive, uint32_t lba, uint32_t count, void *buf) {
    ...
    while (count > 0) {
        ...
        ata_outb(base + ATA_REG_SECCOUNT, (uint8_t)sectors);
        ata_outb(base + ATA_REG_LBA0,     (uint8_t)(lba      ));
        ata_outb(base + ATA_REG_LBA1,     (uint8_t)(lba >>  8));
        ata_outb(base + ATA_REG_LBA2,     (uint8_t)(lba >> 16));
...
int ata_write(int bus, int drive, uint32_t lba, uint32_t count, const void *buf) {
    ...
    while (count > 0) {
        ...
        for (uint32_t s = 0; s < sectors; s++) {
            ...
            for (int w = 0; w < 256; w++)
                ata_outw(base + ATA_REG_DATA, *ptr++);
