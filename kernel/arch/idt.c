...
static inline uint8_t idt_inb(uint16_t port) {
    uint8_t r;
...
static void pic_remap(void) {
    idt_outb(PIC1_CMD,  0x11); io_wait();
    idt_outb(PIC2_CMD,  0x11); io_wait();
    idt_outb(PIC1_DATA, 0x20); io_wait();
    idt_outb(PIC2_DATA, 0x28); io_wait();
    idt_outb(PIC1_DATA, 0x04); io_wait();
...
void idt_init(void) {
    ...
    idt_set(0x23, (uint64_t)irq_stub_3);
    idt_set(0x24, (uint64_t)irq_stub_4);
    idt_set(0x25, (uint64_t)irq_stub_5);
    idt_set(0x26, (uint64_t)irq_stub_6);
    idt_set(0x27, (uint64_t)irq_stub_7);
    idt_set(0x28, (uint64_t)irq_stub_8);
    idt_set(0x29, (uint64_t)irq_stub_9);
    idt_set(0x2A, (uint64_t)irq_stub_10);
    idt_set(0x2B, (uint64_t)irq_stub_11);
    idt_set(0x2C, (uint64_t)irq_stub_12);
    idt_set(0x2D, (uint64_t)irq_stub_13);
