...
#define SB_SECTOR       0               // superblock
#define TABLE_SECTOR    1               // file table starts here
#define TABLE_SECTORS   8               // 8 sectors * 512 = 4096 bytes = 64 entries
#define DATA_START      (TABLE_SECTOR + TABLE_SECTORS)  // sector 9

// Each file entry is 64 bytes, 8 entries per sector
...
// ── Flush entire entry table to disk ─────────────────────────────────────────
static int flush_table(void) {
    // Pack entries into sector-sized buffers and write
    uint8_t buf[512];
    for (uint32_t s = 0; s < TABLE_SECTORS; s++) {
        ...
        for (uint32_t e = 0; e < ENTRIES_PER_SECTOR; e++) {
            ...
            mem_cpy(buf + e * sizeof(vfs_entry_t),
                    &vfs.entries[idx], sizeof(vfs_entry_t));
        }
        if (!ata_write(vfs.bus, vfs.drive, TABLE_SECTOR + s, 1, buf))
            return 0;
    }
    return 1;
}

// ── Load entry table from disk ────────────────────────────────────────────────
static int load_table(void) {
    uint8_t buf[512];
    for (uint32_t s = 0; s < TABLE_SECTORS; s++) {
        if (!ata_read(vfs.bus, vfs.drive, TABLE_SECTOR + s, 1, buf))
            return 0;
        for (uint32_t e = 0; e < ENTRIES_PER_SECTOR; e++) {
            uint32_t idx = s * ENTRIES_PER_SECTOR + e;
            if (idx >= VFS_MAX_FILES) break;
            mem_cpy(&vfs.entries[idx],
                    buf + e * sizeof(vfs_entry_t), sizeof(vfs_entry_t));
        }
    }
    return 1;
}
...
int vfs_create(const char *name, int is_dir) {
    ...
    int idx = -1;
    for (int i = 0; i < VFS_MAX_FILES; i++) {
        if (!vfs.entries[i].used) { idx = i; break; }
    }
    if (idx < 0) return -1;
...
int vfs_write(int idx, const void *data, uint32_t size) {
    ...
    // Pad data to full sectors in a temp buffer (sector-aligned write)
    ...
    for (uint32_t s = 0; s < needed; s++) {
        ...
        if (chunk < ATA_SECTOR_SIZE)
            mem_set(buf + chunk, 0, ATA_SECTOR_SIZE - chunk);

        if (!ata_write(vfs.bus, vfs.drive, e->start_sector + s, 1, buf))
            return 0;
...
int vfs_find(const char *name) {
    for (int i = 0; i < VFS_MAX_FILES; i++) {
        if (vfs.entries[i].used && str_eq(vfs.entries[i].name, name))
            return i;
    }
    return -1;
}
